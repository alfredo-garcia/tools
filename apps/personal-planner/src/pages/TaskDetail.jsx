...
export function TaskDetail() {
  ...
  if (!item) return <p className="text-neutral-500">Tarea no encontrada.</p>
  ...
  return (
    <div className="space-y-6">
      <Link to="/tasks" className="text-sm text-orange-500 dark:text-orange-400 hover:underline">
        ← Volver a Tareas
      </Link>
        ...
        <div className="p-6">
          ...
          {(objLinks.length > 0 || krLinks.length > 0) && (
            <div className="mt-4 pt-4 border-t border-neutral-200 dark:border-neutral-700">
              <h2 className="text-sm font-semibold text-neutral-700 dark:text-neutral-300 mb-2">Vinculado a</h2>
              <div className="flex flex-wrap gap-2">
                {objLinks.map((oid) => (
                  <Link key={oid} to={`/objectives/${oid}`} className="text-orange-500 dark:text-orange-400 hover:underline text-sm">
                    Objetivo
                  </Link>
                ))}
                {krLinks.map((krid) => (
                  <Link key={krid} to={`/key-results/${krid}`} className="text-orange-500 dark:text-orange-400 hover:underline text-sm">
                    Key Result
                  </Link>
                ))}
              </div>
