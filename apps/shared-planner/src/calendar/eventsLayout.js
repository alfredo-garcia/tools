 ...
 * Used by planner views for the Events section. Supports configurable visible range (startHour–endHour).
 */
...
export function getEventsWithColumnLayout(events, dayStr, startHour, endHour) {
  ...
  return result.sort(
    (a, b) => a.slotIndex - b.slotIndex || (a.event.start || '').localeCompare(b.event.start || '')
  )
}
