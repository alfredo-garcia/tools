...
export function TaskDetail() {
  ...
  if (!item) return <p className="text-text-muted">Task not found.</p>
  ...
  const rows = [
    ...
    ['Assignee', str(field(item, 'Assignee', 'Assignee'))],
    ['Due date', dateStr(field(item, 'Due Date', 'Due Date'))],
  ]

  const objLinks = arr(field(item, 'Objectives', 'Objective'))
  const krLinks = arr(field(item, 'Key Result', 'Key Results'))

  const title = str(field(item, 'Task Name', 'Task Name')) || 'Task'
        ...
        <div className="p-6">
          ...
          {(objLinks.length > 0 || krLinks.length > 0) && (
            <div className="mt-4 pt-4 border-t border-border">
              <h2 className="text-sm font-semibold text-text-muted mb-2">Linked to</h2>
